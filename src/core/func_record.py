...
class AndroidRecordManager:
    ...
    def _capture_android_auto_once(
        ...
        elapsed = (time.perf_counter() - start)
        logging.info(f"[ANDROID AUTO SCREENCAP DONE] display={display_id}, elapsed={elapsed:.3f}s")
    ...
    def record_screenshot(self,loca_log=True,save_dir=None):
        ...
        try:
            ...
            elapsed = (time.perf_counter() - start)
            logging.info(f"Screenshot saved successfully: "f"{screenshot_path} elapsed={elapsed:.3f}s")
        except Exception as e:
